...
reads a positional arg. `LabDoctor.check` catches those *cheaply* by probing the
...
both the argument wiring and the metric key. `LabDoctor.fix` then asks Claude to
repair whatever the checks flagged. Collaborators (runner, ui) are injected.
"""
...
class LabDoctor:
    """Diagnoses a lab's wiring (check) and repairs it with Claude (fix)."""

    def __init__(self, lab_dir):
        self._lab = Path(lab_dir)
    ...
    def _probe_args(self, name, cmd_template, checks):
        """A command should receive the file path, not the flag. Returns the probe output."""
        proc, out = self._run(cmd_template, self._lab, _SENTINEL)
        if proc is None:
    ...
    def check(self) -> list[Check]:
        ...
        if (lab / "evaluation.py").exists() and cfg.get("eval_cmd"):
            out = self._probe_args("eval args", cfg["eval_cmd"], checks)
            if out is not None and metric:
                verdict = self._last_json(out)
                if verdict is None:
        ...
        if (lab / "validate.py").exists() and cfg.get("validate_cmd"):
            self._probe_args("validate args", cfg["validate_cmd"], checks)
    ...
    def fix(self, *, runner=None, ui=None, rounds=3) -> bool:
        ...
        for attempt in range(1, rounds + 1):
            ...
            try:
                with ui.status("Claude is editing the lab to fix the issues…"):
                    runner.run_plain(self._fix_prompt(fails))
            finally:
